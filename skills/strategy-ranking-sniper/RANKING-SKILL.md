...
async function mcpCall(toolName, args) {
  // OKX API auth is handled internally by onchainos — no API key needed here
...
2. 创建 `.env`（可选，仅 Telegram 通知等可选配置），引导用户执行 `onchainos wallet login`
...
| 日志中出现 `MCP [100]: xxx` | OKX API 调用失败 | 确认 onchainos 已登录且网络正常；可重新执行 `onchainos wallet login` |
| `Empty ranking` 不断出现 | MCP 排行榜接口返回空数据 | 检查网络连接；确认 API Key 有效；可能是 MCP 服务暂时不可用 |
| Live 模式**买入失败** | WSOL 准备或交易签名失败 | 确认已执行 `onchainos wallet login`；检查钱包 SOL 余额是否充足（至少 `buyAmountPerTrade + 0.01`） |
| `onchainos wallet not available` | Live 模式缺少钱包授权 | 执行 `onchainos wallet login` 完成登录 |
...
- **实盘（Live）**：真实链上交易，需要先执行 `onchainos wallet login`

切换规则：
- 必须先**停止引擎**才能切换模式
- 切换到实盘时，后端会验证 onchainos 钱包是否已登录
...
- **RULE-6**: Live 模式必须完成 `onchainos wallet login`，否则引擎拒绝启动
...
```javascript
// ✅ 正确：通过 onchainos wallet CLI 签名，不持有私钥
import { execSync } from 'child_process';

// 获取钱包地址（onchainos wallet balance）
export function getWalletAddress() {
  const out = execSync('onchainos wallet balance --json').toString();
  return JSON.parse(out).data.solAddress;
}

// ✅ 正确：签名操作委托给 onchainos，不在进程内保存私钥
export async function signAndSend(txBase64) {
  // onchainos wallet contract-call handles sign + broadcast
  // ...
}

// ❌ 错误：绝不做以下操作
// console.log('Private key:', someKey);
// return { keypair: ..., ... };
...
| 规则 | 说明 |
|------|------|
| OKX API 凭证由 onchainos 内部管理 | 不在代码或 `.env` 中配置 OKX API Key |
| API 调用通过 onchainos CLI 代理 | 不直接在应用代码中构造 OKX HTTP 请求头 |
| 调用失败只记录错误码 | 不记录任何凭证信息 |
